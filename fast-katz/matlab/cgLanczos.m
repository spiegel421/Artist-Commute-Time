function [ x, istop, itn, Anorm, Acond, rnorm, xnorm, rhist, D ] = ...
           cgLanczos( A, b, show, check, itnlim, rtol, localsize )
...
%
% 09 Mar 2011: Added optional reorthogonalization for improved estimate of
%              the diagonals of the inverse.
%---------------------------------------------------------------------
...
if ~done                        % k = itn = 1 first time through
  while itn < itnlim
    ...
    if localorth
        ...
        for locali = 1:locallimit
            vtemp = local(:, locali);
            vorth = vorth - (vorth'*vtemp)*vtemp;
        end
        % save vorth
        v = vorth;
    
        % enqueue v now too
        if (localptr < localsize)
            localptr = localptr + 1;
        else
            localptr = 1;
            localqfull = true;
