 ...
 * :2011-03-14: Added balanced pushes (based on residual r/deg)
 */
               ...
               mwSize *out_nedges, mwSize *out_nverts, mwSize *out_nsteps)
{
    ...
    double *d = mxMalloc(sizeof(double)*N);
    
    /* initialize the elements of L to null-values */
    { mwIndex i; for (i=0; i<N; i++) { L[i] = N+1; } }
            
    /* initial d to the degrees */
    { mwIndex i; for (i=0; i<N; ++i) { d[i] = ((double)(ai[i+1]-ai[i])); } }
    ...
    while (steps < maxsteps) {
        ...
        val = r[v]*d[v]; /* r[v] is always r[v]/d[v] */
        x[v] = x[v] + omega*val;
        r[v] = (val - omega*val)/d[v]; /* scale r[v] by /d[v] */
        val *= omega/((double)(d[v])); /* value to propagate to neighbors */
